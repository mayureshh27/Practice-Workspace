...
import { ingestSource, type IngestFileRequestSchema } from './sourcesApi';
...
export const sourcesQueries = {
  list: () =>
    queryOptions({
      ...
      staleTime: 1000 * 60 * 5, // Shorter: ingest makes this stale
    }),
  // Phase 10 addition:
  ingest: (body: z.infer<typeof IngestFileRequestSchema> & { content?: string }) => ({
    mutationKey: ['sources', 'ingest'],
    mutationFn: () => ingestSource(body),
  }),
};
