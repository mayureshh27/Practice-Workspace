...
import { problemsQueries, domainQueries } from '../../../../../api/queries';
import {
  getBrowserStorage,
  readJsonStorage,
  readTextStorage,
...
function TopicRoute() {
  ...
  const domainKey =
    domainId === 'go-programming' ? 'go' : domainId === 'robotics' ? 'robotics' : undefined;
  ...
  const select = (p: Problem) => {
    ...
    setUploaded(
      readJsonStorage(
        ...
        (v): v is UploadedFile[] =>
          Array.isArray(v) &&
          v.every((x) => x && typeof x === 'object' && typeof (x as any).name === 'string'),
      ),
    );
    setProofReady(false);
    setOut('');
  ...
  useEffect(() => {
    if (store && store.problems.length > 0) {
      const p = store.problems.find(
        (prob) =>
          prob.id.toLowerCase() === topicId.toLowerCase() ||
  ...
  const resetDraft = () => {
    ...
    if (!window.confirm('Reset editor to the starter code? Your current draft will be lost.'))
      return;
  ...
  const filtered = useMemo(() => {
    ...
    return store.problems.filter((p) => {
      ...
      const qMatch =
        !q || p.title.toLowerCase().includes(q) || p.statement.toLowerCase().includes(q);
