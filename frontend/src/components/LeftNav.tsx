...
function LeftNav() {
  ...
  const { mutate: onRenameDomain } = useRenameDomainMutation();
  const { mutate: onDeleteDomain } = useDeleteDomainMutation();
  const { mutate: onTogglePinDomain } = useTogglePinDomainMutation();
  const { mutate: onToggleArchiveDomain } = useToggleArchiveDomainMutation();
  ...
  const finishRename = () => {
    if (renaming && renameValue.trim()) {
      onRenameDomain({ id: renaming, name: renameValue.trim() });
    }
    setRenaming(null);
  };
  ...
  void activeDomainFromPath;
  void activeSubjectFromPath; // referenced in JSX below

  const renderRecentItem = (item: RecentItem, paddingLeft: number = 8) => {
    let toStr = '/';
    let paramsObj: any = {};
    if (item.loc.level === 'domain') {
      toStr = '/domain/$domainId';
      paramsObj = { domainId: item.loc.domainId };
    } else if (item.loc.level === 'subject') {
      toStr = '/subject/$domainId/$subjectId';
      paramsObj = { domainId: item.loc.domainId, subjectId: item.loc.subjectId };
    } else if (item.loc.level === 'chapter') {
      toStr = '/chapter/$domainId/$subjectId/$chapterId';
      paramsObj = {
        domainId: item.loc.domainId,
        subjectId: item.loc.subjectId,
        chapterId: item.loc.chapterId,
      };
    } else if (item.loc.level === 'topic') {
      toStr = '/topic/$domainId/$subjectId/$chapterId/$topicId';
      paramsObj = {
        domainId: item.loc.domainId,
        subjectId: item.loc.subjectId,
        chapterId: item.loc.chapterId,
        topicId: item.loc.topicId,
      };
    }
  ...
  return (
    <nav
      className={`flex flex-col bg-ws-bg border-r border-ws-line transition-[width] duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] ${collapsed ? 'w-12' : 'w-[220px]'}`}
    ...
    >
      ...
      {collapsed ? (
        ...
        <div className="flex items-center justify-between h-11 px-3 shrink-0">
          <span className="font-bold text-[13px] text-ws-ink tracking-tight">
            Practice Workspace
          </span>
          <button
            type="button"
      ...
      {!collapsed && (
        <div className="flex items-center gap-1 p-2">
          <button
            type="button"
            className={`press transition-colors flex items-center justify-center rounded hover:bg-ws-surface-2 text-ws-muted hover:text-ws-ink w-6 h-6 ${canGoBack ? 'opacity-100' : 'opacity-30'}`}
            onClick={onBack}
            disabled={!canGoBack}
            title="Back"
          >
          ...
          <button
            type="button"
            className={`press transition-colors flex items-center justify-center rounded hover:bg-ws-surface-2 text-ws-muted hover:text-ws-ink w-6 h-6 ${canGoForward ? 'opacity-100' : 'opacity-30'}`}
            onClick={onForward}
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden min-h-0 p-2 scrollbar">
        ...
        {collapsed ? (
          (() => {
            const isDomainsActive = ['/', '/domain', '/subject', '/chapter', '/topic'].some(
              (p) =>
                location.pathname.startsWith(p.length > 1 ? p : '/') &&
                (p === '/'
                  ? location.pathname === '/' ||
            ...
            return (
              <>
                ...
                >
                  <Folder
                    size={16}
                    className={isDomainsActive ? 'text-ws-accent' : 'text-ws-soft'}
                  />
                </Link>
          ...
          <div className="flex items-center px-3 h-8 text-ws-muted text-[13px]">Loading...</div>
        ) : (
              ...
              <div key={domain.id} className="mb-1">
                ...
                <div
                  ...
                  onClick={() => {
                    ...
                    navigate({ to: '/domain/$domainId', params: { domainId: domain.id } });
                  }}
                ...
                >
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      onOpenCreateModal('subject', domain.id);
                    }}
                    title="Add subject"
                      ...
                      <div key={subject.id} className="ml-3.5 mb-0.5">
                        ...
                        >
                          ...
                          <span className="text-ws-muted flex shrink-0">
                            <BookOpen
                              size={14}
                              className={isSubActive ? 'text-ws-accent' : 'text-ws-muted'}
                            />
                          </span>
                          <span
                            className={`text-[13px] font-medium flex-1 overflow-hidden text-ellipsis whitespace-nowrap min-w-0 ${isSubActive ? 'text-ws-ink' : 'text-ws-soft'}`}
                          >
                            {subject.name}
                          </span>
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              onOpenCreateModal('chapter', domain.id, subject.id);
                            }}
                            title="Add chapter"
                            className="press bg-transparent border-none text-ws-muted hover:text-ws-accent flex shrink-0 p-0.5 opacity-60 hover:opacity-100 transition-colors"
                          >
                        ...
                        {isSubExpanded &&
                          subject.chapters.map((chapter) => {
                            ...
                            const isChActive =
                              location.pathname ===
                              `/chapter/${domain.id}/${subject.id}/${chapter.id}`;
                              ...
                              <div key={chapter.id} className="ml-3.5 mb-0.5">
                                ...
                                >
                                  ...
                                  <span className="text-ws-muted flex shrink-0">
                                    <Layers
                                      size={14}
                                      className={isChActive ? 'text-ws-accent' : 'text-ws-muted'}
                                    />
                                  </span>
                                  <span
                                    className={`text-[13px] font-medium flex-1 overflow-hidden text-ellipsis whitespace-nowrap min-w-0 ${isChActive ? 'text-ws-ink' : 'text-ws-soft'}`}
                                  >
                                    {chapter.name}
                                  </span>
                                </div>
                                ...
                                {isChExpanded &&
                                  chapter.topics.map((topic) => {
                                    const isTopicActive =
                                      location.pathname ===
                                      `/topic/${domain.id}/${subject.id}/${chapter.id}/${topic.id}`;
                                      ...
                                      >
                                        <span className="text-ws-muted flex shrink-0">
                                          <FileCode
                                            size={14}
                                            className={
                                              isTopicActive ? 'text-ws-accent' : 'text-ws-muted'
                                            }
                                          />
                                        </span>
                                        <span
                                          className={`text-[13px] flex-1 overflow-hidden text-ellipsis whitespace-nowrap min-w-0 ${isTopicActive ? 'text-ws-ink' : 'text-ws-muted'} ${isTopicActive ? 'font-medium' : 'font-normal'}`}
                                        >
                                          {topic.name}
                                        </span>
                                      </div>
        ...
        <div className="mt-4 border-t border-ws-edge-soft pt-2">
          {!collapsed && (
            <div className="text-[10px] font-bold text-ws-muted uppercase tracking-wider px-3 mb-2 mt-4">
              Tools
            </div>
          )}
          <Link
              ...
              <div className="relative">
                ...
                {showRecentMenu && (
                  <>
                    ...
                    />
                    <div
                      className="pop-pop origin-trigger-tr absolute right-0 top-5.5 z-[1000] bg-ws-bg border border-ws-line rounded-md p-1 min-w-[120px] shadow-2xl"
                      onClick={(e) => e.stopPropagation()}
                    >
                      <div className="px-2 py-1.5 text-[10px] text-ws-muted uppercase tracking-wider">
                        Group by
                      </div>
            ...
            {[
              {
                ...
                action: () => {
                  const d = domains.find((x) => x.id === contextMenu.id);
                  if (d) onTogglePinDomain({ id: d.id, pinned: !d.pinned });
                  closeContext();
                },
              },
              {
                icon: Archive,
                label: domains.find((d) => d.id === contextMenu.id)?.archived
                  ? 'Unarchive'
                  : 'Archive',
                action: () => {
                  const d = domains.find((x) => x.id === contextMenu.id);
                  if (d) onToggleArchiveDomain({ id: d.id, archived: !d.archived });
                  closeContext();
                },
              },
