...
func main() {
	...
	port := env("PORT", "8080")
	fmt.Println("GoPrac backend on :" + port)
...
func runCode(w http.ResponseWriter, r *http.Request) {
	...
	if p == nil {
		http.Error(w, "unknown problem", 404)
		return
	}
	start := time.Now()
...
func execute(code, tests, mode string) (string, error) {
	...
	if mode == "submit" {
		...
		if err := os.WriteFile(filepath.Join(dir, "judge.go"), []byte(judge), 0600); err != nil {
			return "", err
		}
	}
	...
	if mode == "submit" {
		args = []string{"run", file, filepath.Join(dir, "judge.go")}
	}
	...
	if err != nil {
		return buf.String(), fmt.Errorf("%s", strings.TrimSpace(buf.String()))
	}
	if buf.Len() == 0 {
		if mode == "submit" {
